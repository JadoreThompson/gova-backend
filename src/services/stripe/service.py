...
from .exceptions import StripeVerificationError


class StripeService:
    _handlers: dict | None = None
    _email_service: BrevoEmailService | None = None
    _kafka_producer: AIOKafkaProducer | None = None
    _logger = logging.getLogger("StripeService")
    ...
    async def handle_event(cls, event_bytes: bytes, sig_header: str) -> bool:
        ...
        if not cls._email_service:
            cls._email_service = BrevoEmailService("Gova", "no-replay@gova.chat")

        try:
            event = stripe.Webhook.construct_event(
                payload=event_bytes,
                sig_header=sig_header,
                secret=STRIPE_PRICING_PRO_WEBHOOK_SECRET,
            )
        except stripe.SignatureStripeVerificationError:
            raise StripeVerificationError("Invalid signature.")
        ...
        cls._logger.warning(f"Handler for event type '{event_type}' not found.")
        return False
